...
29},city={Manchester,},postcode={2054 NX},country={The Netherlands},}
  ...
  complicates efforts to address transport poverty. This study presents
  a model-based method to develop equity standards by exploring the
  relationship between calculated and perceived job accessibility for
  individuals living in the Greater Toronto and Hamilton Area, Canada.
  We combined place-based accessibility indicators (active
  transportation, public transit, and motorized modes) with
  individual-level variables to estimate weighted regression models. We
  controlled for endogeneity using spatial filters as instrumental
  variables. For the perception of having too few jobs available within
  an ideal commuting time, we found that increasing calculated transit
  accessibility reduced negative perceptions. For active commuters,
  active accessibility also showed a slight positive effect on this
  perception, while also slightly reducing the probability of reporting
  that employment opportunities had been declined due to transport
  constraints. Based on the public transit results, we identified
  accessibility thresholds that increase the likelihood of positive
  perceptions for equity-deserving groups and classified them as
  conservative, intermediate, and progressive equity standards. About
  60\% of the study area population lives below the conservative
  standard, 91\% below the intermediate standard, and nearly the entire
...
of moving \citep{pereira2023}; instead, they travel to access
opportunities, activities, or other people at their destinations.
...
accessibility complicates efforts to address transportation poverty
...
calculated job accessibility be related to perceived accessibility of
...
Our research advances the literature on the equitable transportation
benefit distribution by proposing a method that identifies calculated
accessibility standards based on their relationship with perceived
accessibility - more specifically, with perceived employment
reachability. The proposed standards are derived at the neighbourhood
level, while perceived reachability reflects the experiences of
individuals residing in those areas, also depending on population
characteristics. In this way, these thresholds can assist policymakers
in interpreting how different levels of calculated accessibility are
associated with residents' perceived ability to reach employment
opportunities under existing social and spatial constraints. This
increases the potential for government adoption and enhances the
comprehensibility and relevance of such standards for community members
and those experiencing unequal mobility. Furthermore, identifying
sufficient job accessibility levels can help assess and highlight areas
at higher risk of transport poverty, particularly those with a higher
concentration of equity-deserving populations.
...
accessible through our GitHub page (\emph{include after review}), in
alignment with best practices in spatial data science
\citep{arribas-bel2021}.
...
Accessibility standards typically combine a fairness conceptualization
with threshold criteria, reflecting both vertical and horizontal equity
...
users (e.g., a maximum acceptable distance to a transit stop). After a
systematic review, Souhkov et al.\citeyearpar{soukhov2025a} identified
that accessibility standards are the most commonly applied in
transportation literature (68\%).
...
non-normative standards based on key thresholds of calculated
accessibility.

\section{Materials and Methods}\label{materials-and-methods}

Our methodology for obtaining equity standards of accessibility based on
...
geographic resolution (that is, household geographic coordinates) that
enables the linking with calculated accessibility.
...
I have had to decline employment opportunities because of my transport situation & 41.41 (31.30)\\
\bottomrule
...
likelihood that the trip will occur given a specif travel time. We
...
\citeyearpar{huntington-klein2021}, we drew a causal diagram to
...
the \texttt{gtsummary\ R\ package} \citep{gtsummary}).
...
Motorized job accessibility per user & 1.00 (0.27)\\
...
this study, we propose to address the potential endogeneity by applying
an instrumental variable (IV) technique. An instrumental variable \(Z\)
...
outcome (in its continuous form), which is consistent with the relevance
and exclusion conditions required for valid instrumental variables. In
addition, we performed the Wald test to evaluate the null hypothesis of
...
\midrule
Active transportation job accessibility per user & \textbf{0.86*} & 0.064 & 0.024 & \textbf{0.84*} & 0.082 & 0.035\\
Transit job accessibility per user & \textbf{1.25*} & 0.112 & 0.044 & 0.87 & 0.189 & 0.5\\
Motorized job accessibility per user & 0.93 & 0.097 & 0.4 & 1.19 & 0.147 & 0.2\\
Number of Cars & 0.98 & 0.121 & 0.9 & 1.11 & 0.136 & 0.5\\
Number of Bikes & 1.08 & 0.067 & 0.2 & 1.02 & 0.065 & 0.8\\
\addlinespace
Preferred mode match most frequent mode & 1.14 & 0.168 & 0.4 & 1.30 & 0.246 & 0.3\\
Montly or annual public transit pass & \textbf{0.68*} & 0.174 & 0.026 & \textbf{0.52*} & 0.256 & 0.011\\
Public transit availability in the neighbourhood & 0.75 & 0.245 & 0.3 & 1.05 & 0.226 & 0.8\\
Indigenous or racialized person & \textbf{0.60**} & 0.164 & 0.002 & \textbf{0.57**} & 0.210 & 0.007\\
Temporary visa or refugee & 1.61 & 0.257 & 0.064 & 0.71 & 0.415 & 0.4\\
\addlinespace
Person with disability & 0.69 & 0.209 & 0.077 & 0.80 & 0.229 & 0.3\\
Woman, transgender or non-binary person & 1.05 & 0.149 & 0.7 & 1.03 & 0.194 & 0.9\\
Age & 1.01 & 0.006 & 0.080 & \textbf{1.02*} & 0.007 & 0.017\\
Household size & 1.06 & 0.052 & 0.2 & 0.94 & 0.067 & 0.3\\
Household income &  &  & 0.11 & \textbf{**} &  & 0.003\\
\addlinespace
\hspace{1em}Less than \$40,000 & — & — &  & — & — & \\
\hspace{1em}\$40,000 to \$80,000 & 0.80 & 0.188 &  & 1.48 & 0.270 & \\
\hspace{1em}\$80,000 to \$120,000 & 1.23 & 0.220 &  & 1.97 & 0.248 & \\
\hspace{1em}More than \$120,000 & 1.29 & 0.227 &  & 3.08 & 0.312 & \\
Population density (people/km2) & \textbf{1.18*} & 0.075 & 0.024 & 1.10 & 0.100 & 0.3\\
\addlinespace
Population centre class &  &  & 0.3 & \textbf{***} &  & <0.001\\
\hspace{1em}Rural & — & — &  & — & — & \\
\hspace{1em}Small & 1.34 & 0.487 &  & 0.88 & 0.478 & \\
\hspace{1em}Medium & 2.35 & 0.469 &  & 5.07 & 0.452 & \\
\hspace{1em}Large & 1.25 & 0.322 &  & 1.16 & 0.330 & \\
\addlinespace
Residuals: Active acessibility & 1.09 & 0.078 & 0.3 & 0.97 & 0.085 & 0.7\\
Residuals: Transit acessibility & 0.85 & 0.092 & 0.073 & 0.86 & 0.147 & 0.3\\
Residuals: Motorized acessibility & 1.13 & 0.068 & 0.075 & 0.96 & 0.095 & 0.7\\
Active mode interaction: accessibility × user & \textbf{1.17*} & 0.072 & 0.029 & \textbf{1.20*} & 0.088 & 0.034\\
Transit mode interaction: accessibility × user & 0.95 & 0.133 & 0.7 & 1.17 & 0.204 & 0.4\\
\bottomrule
...
for any of the models, probably due to its homogeneous spatial pattern
across the study area. In addition, we believe that the accessibility
indicator for the motorized mode may be overestimated, since the travel
times calculated during the routing process do not account for traffic
congestion, which would otherwise reduce vehicle speeds and,
consequently, accessibility levels.

The model related to ``there are too few jobs available for me in my
ideal commuting time'' was the one that best captured the expected
behaviour of accessibility, in which increasing transit and active
calculated accessibilities implies a higher probability of being in more
positive perception categories. In fact, this is the model in which the
outcome is the most similar to the spatial availability of jobs
definition, which refers directly to the number of jobs accessible,
...
Active accessibility plays an overall significant and negative effect,
reducing the odds of being in a more positive perception category by
14\%. However, as shown in Figure \ref{fig:active-marginal-effects},
when looking at the interaction coefficients between accessibility and
active users, we observe that the negative effect is actually cancelled
and the net effect of this accessibility mode becomes slightly positive
(0.62\% increase). This same effect of a negative coefficient for active
accessibility, but a positive net effect when considering the
interaction with the transportation mode used, also occurs for the model
related to having to decline employment opportunities because of
transport situation - a 16\% reduction for non-active commuters, but
0.08\% increase for active commuters. We believe that the general
negative effect of active accessibility for non-active commuters can be
...
availability, but due to a very low number of active commuters,
...
\ref{fig:accessibility-effect} displays the specific effect of
increasing transit accessibility across the selected individual profiles
for the ``few jobs available'' model. For all of them, we observe a
decrease in negative perceptions as calculated transit accessibility
increases. For the equity-deserving individual profile, the ``Most
...
categories (``Most negative'' and ``Negative'') together account for
over 52\% of the total probability, resulting in a high likelihood that
individuals in the equity-deserving group to have negative perceptions
at this accessibility level. For the most common population, the initial
pattern appears similar to the equity-deserving group, but the decline
in negative perceptions occurs faster, with the ``Most positive''
probability surpassing negative probabilities at approximately 0.28 jobs
per transit user. In contrast, for the advantaged population, we can see
that even at very low levels of accessibility, the positive perceptions
already present the highest probabilities and increasing transit
accessibility only reinforces these positive perceptions.
...
Both models were used to identify active accessibility thresholds, while
only the ``there are too few jobs available for me in my ideal commuting
time'' model was selected to identify transit thresholds - since
calculated transit accessibility was statistically significant only in
this model.
...
\begin{figure}
\includegraphics[width=1\linewidth]{figures/accessibility-threshold} \caption{Public transit accessibility thresholds based on selected individual profiles.}\label{fig:transit-accessibility-thresholds}
\end{figure}
...
Evaluating the thresholds for active accessibility, we again find that a
level of 0 jobs per active commuter is already enough to result in
positive perceptions for the most advantaged profile in both models,
meaning that the current active accessibility levels would be sufficient
and no individuals would fall below the minimum accessibility threshold.
For the most common profile, the identified conservative threshold is
equal to 0 jobs per active user for both modes; and the progressive
thresholds are approximately 1.6 and 3.4 jobs per active user, resulting
in higher probabilities of positive perceptions for the models of
declining employment and few jobs available, respectively.
...
\midrule
(Intercept) & \textbf{79***} & 7.42 & <0.001 & \textbf{56***} & 7.63 & <0.001\\
Predicted active accessibility per user & 2.1 & 3.10 & 0.5 & \textbf{13*} & 5.21 & 0.013\\
Predicted transit accessibility per user & \textbf{-76***} & 18.2 & <0.001 & 7.6 & 23.6 & 0.7\\
Predicted motorized accessibility per user & -6.6 & 5.20 & 0.2 & -2.3 & 6.59 & 0.7\\
Number of Cars & 0.65 & 1.54 & 0.7 & -1.5 & 1.36 & 0.3\\
\addlinespace
Number of Bikes & -1.5 & 0.947 & 0.11 & 0.79 & 0.795 & 0.3\\
Preferred mode match most frequent mode & -0.29 & 2.10 & 0.9 & \textbf{-5.6**} & 2.15 & 0.010\\
Montly or annual public transit pass & \textbf{5.1*} & 2.35 & 0.030 & \textbf{15***} & 2.80 & <0.001\\
Public transit availability in the neighbourhood & \textbf{6.8*} & 3.39 & 0.045 & -3.8 & 2.83 & 0.2\\
Indigenous or racialized person & \textbf{12***} & 2.20 & <0.001 & \textbf{17***} & 2.24 & <0.001\\
\addlinespace
Temporary visa or refugee & \textbf{-9.1*} & 3.86 & 0.019 & 2.7 & 4.37 & 0.5\\
Person with disability & \textbf{6.7**} & 2.39 & 0.005 & 2.9 & 2.40 & 0.2\\
Woman, transgender or non-binary person & 2.3 & 1.96 & 0.2 & 0.69 & 2.05 & 0.7\\
Age & \textbf{-0.16*} & 0.081 & 0.041 & \textbf{-0.16*} & 0.075 & 0.035\\
Household size & -1.3 & 0.653 & 0.054 & 0.76 & 0.737 & 0.3\\
\addlinespace
Household income & \textbf{***} &  & <0.001 & \textbf{***} &  & <0.001\\
\hspace{1em}Less than \$40,000 & — & — &  & — & — & \\
\hspace{1em}\$40,000 to \$80,000 & 1.7 & 2.34 &  & -5.8 & 2.51 & \\
\hspace{1em}\$80,000 to \$120,000 & -7.5 & 2.85 &  & -15 & 2.98 & \\
\hspace{1em}More than \$120,000 & -9.9 & 2.79 &  & -27 & 3.15 & \\
\addlinespace
Population density (people/km2) & \textbf{0.00**} & 0.000 & 0.006 & \textbf{0.00**} & 0.000 & 0.001\\
Population centre class & \textbf{**} &  & 0.008 & \textbf{***} &  & <0.001\\
\hspace{1em}Rural & — & — &  & — & — & \\
\hspace{1em}Small & -8.5 & 7.41 &  & 11 & 6.12 & \\
\hspace{1em}Medium & -18 & 5.95 &  & -17 & 4.66 & \\
\addlinespace
\hspace{1em}Large & -1.3 & 3.86 &  & -4.9 & 4.03 & \\
Active mode interaction: accessibility × user & \textbf{-12*} & 5.47 & 0.027 & -0.06 & 8.32 & >0.9\\
Transit mode interaction: accessibility × user & 22 & 15.5 & 0.2 & \textbf{44*} & 18.7 & 0.018\\
\bottomrule
