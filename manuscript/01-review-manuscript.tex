...
\hypersetup{breaklinks=true,
            bookmarks=true,
            pdfauthor={},
            pdftitle={Exploring equity standards for transport policy: A case study of calculated accessibility, perceived accessibility, and employment outcomes in the Greater Toronto and Hamilton Area, Canada},
            colorlinks=false,
            urlcolor=blue,
            linkcolor=magenta,
            pdfborder={0 0 0}}
...
of calculated accessibility, perceived accessibility, and employment
outcomes in the Greater Toronto and Hamilton Area, Canada}
...
3K7},state={Ontario},country={Canada},}
  ...
  calculated accessibility, perceived job accessibility, and employment
  outcomes for individuals living in the Greater Toronto and Hamilton
  Area, Canada. We use calculated accessibility for three modes of
  travel (active transportation, public transit, and private vehicles)
  along individual-level variables to estimate ordinal models. To
  control for endogeneity we use spatial filters as instrumental
...
is worth noting, however, it is important to mention that congestion is
part of the everyday routine of many people and one of the most
immediate and felt transportation concerns of the public
\citep{handy2020}, which may help to explain why mobility-centered
approaches remain in practice.

Access to opportunities is one of the main ways in which transportation
can help in the promotion of a dignified life \citep{karner2020, BRUNO}.
...
based on the relationship between accessibility calculated based on
spatial and transport data - which we refer to hereafter as calculated
accessibility - and people's perceived accessibility of job
opportunities and self-reported employment outcomes, grounded in an
egalitarian and sufficientarian understanding of transportation equity.
From this perspective, a fair transportation system should guarantee a
minimum, sufficient level of accessibility to all members of society,
with this sufficient level being defined from the standpoint of those
facing the greatest transportation disadvantage (Martens and Golub,
2021; Pereira and Karner, 2021).

While prior studies have examined the association between calculated and
perceived accessibility, and between calculated accessibility and
employment outcomes, few have used these relationships to derive
accessibility thresholds as propositions towards equitable
transportation planning. Our research advances the literature on the
equitable distribution of transportation benefits by proposing a
model-based approach to identify minimum levels of calculated
accessibility associated with reduced likelihood of negative perceived
accessibility and employment outcomes. This research is made possible by
the release of Canada's first National Survey of Transport Poverty
(NSTP) \citep{tiznado-aitken}. Although the proposed standards are
presented at the neighbourhood level, they indirectly also incorporate
individual-level characteristics, since these standards are derived from
individuals' self-reported perceptions and outcomes in the NSTP. In this
way, these thresholds can assist policymakers in interpreting how
...
poverty, particularly those with a higher concentration of transport
...
classified as a place-based measure \citep{pereira2023}, representing a
...
\citep{Baber}. In this situation, someone with restricted financial
...
abilities, and travel-related attitudes. In fact, residential location
decisions differ across income groups, impacting the relationship
between calculated job accessibility and employment status (cite Hu,
Cao, Cervero). For instance, high-income individuals often prioritize
...
to reach employment opportunities may be shaped less by their
...
contributing to the somewhat limited knowledge on the topic
\citep{negm2025}. In this context, connecting calculated to perceived
accessibility helps to capture non-spatial and employment‑related
barriers that are not visible in place‑based measures alone, since the
calculated accessibility might be perceived differently even between
people living within that same neighbourhood. In this paper, as
suggested by Pot et al. \citeyearpar{pot2025}, we define sufficient
levels of accessibility based on the relationship between calculated
accessibility and people's perception of job accessibility and
employment outcomes, while also controlling for individual
characteristics.
...
transport-related benefits. Pereira and Karner (2021) argue that
transportation equity research benefits from linking two justice
theories: Rawlsian egalitarianism (Rawls, 1999) and the Capability
Approach (Sen, 1999, 2005). Rawls' Difference Principle holds that
...
rarely made explicit, in transportation equity research (Martens, 2016b;
Wee and Geurs, 2011). The sufficientarian perspective asks why some
groups or areas lack sufficient access, building on the idea of a
...
group's absolute access level meets a minimum threshold (Pereira and
Karner, 2021).
...
dignified life \citep{karner2020}, and the one of main benefits of
transportation to be distributed across society in order to increase
people's freedom. we adopt an perspective that combines both
sufficientarian and egalitarian principles. Following the
sufficientarian perspective, we define equity standards as minimum
accessibility thresholds required to reduce the likelihood of negative
perceptions and outcomes. Following the egalitarian view, we define
these thresholds based on transport disadvantage population groups,
rather than in general population, aiming to reduce inequalities of
opportunities between population groups.
...
\citep{cancensus}). According to the Census
...
\(m\) competing for the opportunities \(O\) at the destination \(_j\).
In this study the population competing for these opportunities refers to
people in the labour force according classification used in the Canadian
Census.
...
the jobs available, while the remaining 0.6\% percent of jobs available
are competed for by the 7\% of active users. On average, each active
user respondent from the NSTP can access 0.06 jobs, while each transit
user can access, on average, 0.14 jobs. These both measures are
substantially lower than the number of jobs that each private vehicle
commuter can reach, which is approximately one job per commuter on
average.
...
accessibility increases. For the transport disadvantage individual
profile, the ``Most negative'' perception has high probability at low
...
high likelihood that individuals in the transport disadvantage group to
have negative perceptions at this accessibility level. For the most
common population, the initial pattern appears similar to the transport
disadvantage group, but the decline in negative perceptions occurs
faster, with the ``Most positive'' probability surpassing negative
probabilities at approximately 0.28 jobs per transit user. In contrast,
for the advantaged population, we can see that even at very low levels
of accessibility, the positive perceptions already present the highest
...
\toprule
Variable & Transport disadvantage & Most common population & Advantaged group\\
\midrule
...
their equity-weighted AFI, for each standard type. All census
...
only 35\% of the CTs (516 out of the 1,489 CTs in the study area)
present transit accessibility levels above the shortfall limit, with
most of them (83\%) located in the City of Toronto. This share of CTs
without an accessibility shortfall decreases to less than 10\% for the
intermediate equity standard (136 CTs). Only one CT, located in Toronto,
does not present an accessibility shortfall when evaluated under the
progressive standard. This unique CT corresponds to the area surrounding
the St.~George Subway Station in Downtown Toronto, connected to the two
subway lines that cross the city and link neighbouring municipalities,
running from north to south and from east to west. This CT is bounded by
Spadina Avenue to the west and Bloor Street to the south, with many
connections to the public transit system, including a streetcar line on
Spadina.
...
shifts. A list of the total number of people living in each priority
area, by equity standard and subdivision, is available as supplementary
material.
...
and perceived accessibility for this opportunity type. We believe,
however, that our methodology can be extended to other opportunity types
essential for a meaningful life, such as healthcare and education,
provided that the analyst has the necessary data on hand to perform the
analysis.
...
mobility experiences or overall transport conditions. Rather, the
...
an object of a specific method used to categorized our dependent
variable.
...
case in instrumental variable designs {[}cite{]}, our assumption that
the instruments affect perceived accessibility and employment outcomes
...
applicable to other regions. By definition, accessibility is dependent
on the study area's land-use and transport characteristics, and we would
not expect or intend for our thresholds to be transferred elsewhere. We
intend to suggest minimum levels of accessibility that could guide
transportation planning towards a more equitable distribution of
transportation benefits, as an earlier stage to be succeeded by a
participatory and democratic process to collectively select the best
standard, considering that residents and other stakeholders must be able
to actively participate in and influence the decisions that affect their
lives. While this is, in our perspective, the most appropriate method
for standards definition, it is beyond the scope of this study to
develop democratic plenaries to discuss the suggested standards.
...
positive effect on individual perception, increasing their odds of being
in a positive category. Using the perception-based model and for the
public transit mode, we suggest equity standards based on the
accessibility threshold that results in a higher likelihood of positive
perceptions for people from transport-disadvantaged groups. We suggest
three equity standards: a conservative standard (0.16 jobs reached by
public transit per transit user), an intermediate standard (0.26), and a
