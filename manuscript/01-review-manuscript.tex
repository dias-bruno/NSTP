...
3K7},state={Ontario},country={Canada},}
  ...
  using spatial filters as instrumental variables to account for
  potential confounding. We examine the association between calculated
  accessibility and the perception of having too few jobs available
  within an ideal commuting time (a perceived accessibility indicator)
  and the reports of having to decline jobs because of transportation
  constraints (an employment outcome indicator). The results indicate
  that higher levels of active and transit accessibility are associated
  with more positive perceptions and outcomes regarding these
  ...
  perceptions and outcomes for transport-disadvantaged groups. We
...
\subsection{Modeling the relationship between calculated accessibility,
perceived accessibility, and employment
outcomes}\label{modeling-the-relationship-between-calculated-accessibility-perceived-accessibility-and-employment-outcomes}
...
perceived accessibility and employment outcomes. The underlying
hypothesis of this strategy is that changes in the land-use and
...
with more (calculated) accessibility are less likely to perceive job
...
\citeyearpar{huntington-klein2021}, we draw a Directed Acyclic Graph
(DAG) to graphically represent our hypothesis (Figure \ref{fig:DAG}).
DAGs show a chain of hypothesized causal effects by using nodes to
...
\begin{figure}
\includegraphics[width=1\linewidth]{figures/DAG} \caption{Directed acyclic graph (DAG) of calculated accessibility on perceived accessibility.}\label{fig:DAG}
\end{figure}

We hypothesize that there is a association between calculated job
accessibility, perceived accessibility, employment outcomes. However,
there are other ways in which they are connected, represented by the
other nodes of the diagram. First, there are mediator components that
influence the way a person perceives, or make use of, calculated
accessibility. Each individual perceives the same neighbourhood
...
In addition, place of residence, and consequently an individual's
neighbourhood accessibility, is also influenced by socioeconomic
...
However, socioeconomic and demographic factors and the urban landscape
of the place of residence are also linked through a common cause
(\(U_1\)) that we cannot measure directly, such as generational wealth,
which likely influences both someone's neighbourhood and their income
level. Other unmeasured factors, such as residential self-selection and
adaptive preference, likely affect calculated accessibility, perceived
accessibility, and employment outcomes (\(U_2\)).

Because other paths link calculated accessibility to perceived
accessibility and employment outcomes, the observed association between
...
a framework using instrumental variables.

Guided by the diagram and the supporting literature, we identified
control variables corresponding to each node, following standard
causal-diagram identification methods
\citep{cunningham2021causal, huntington-klein2021}. We selected the
following variables:
...
We use weighted regressions that combine multimodal spatial availability
...
relationship between the calculated accessibility indicators and the
categories of the dependent variables, using Equation
...
The presence of \(U_2\) in our DAG implies the possibility of bias
caused by omitted variables that simultaneously affect calculated
accessibility, perceived accessibility, and employment outcomes - a
source of endogeneity that compromises the consistency of the estimators
\citep{antonakis2010, cunningham2021causal, huntington-klein2021}. We
address this using an instrumental variable (IV) technique, where an
instrument \(Z\) must be correlated with the outcome only through
calculated accessibility. Examples of instrumental variables used in
...
Two challenges arise in the search for relevant instruments: the
likelihood of correlation between instruments and the error term
increases with the strength of their correlation with the endogenous
variable, while instruments only weakly correlated with the endogenous
variable tend to perform poorly. Le Gallo and Páez
...
data \citep[see][]{getis_comparative_2002, griffith_spatial_2003}, which
ensures high correlation with the treatment while, potentially,
remaining exogenous to the outcome. Following this approach:
...
\item
  ...
  multiplied the eigenvector by its coefficient and added it to the
  value of \(S\). After this process, \(S\) represents the spatial
...
transforming them into categorical variables), consistent with the
relevance and exclusion conditions required for valid instruments. In
addition, we performed Wald tests to evaluate the null hypothesis of
weak instruments and insufficient explanatory power to predict
calculated accessibility, and Wu-Hausman tests to assess whether the
...
accessibility indicators using a linear regression including all control
...
the first-stage regressions. Both stages used respondent weights. In
...
calculated accessibility to perceived accessibility and employment
outcome. Based on Wald tests, we reject the null hypothesis of weak
instruments and insufficient explanatory power to predict our endogenous
variables (the calculated accessibility indicators), as all F-statistics
are well above the Stock--Yogo critical values and statistically
significant (p-value \textless{} 0.05). In the categorical models, the
first-stage residuals were non-significant in the second-stage
regressions, suggesting little evidence of endogeneity in the calculated
accessibility indicators, and that standard regressions without the IV
approach could also have been appropriate. However, a Wu-Hausman test on
...
our analysis rather than reverting to standard regressions.
...
positive perception and employment outcome: an approach that intervenes
...
that transport-disadvantaged groups have a better chance of experiencing
...
Additionally, the non-significant residuals in the categorical models
...
loss of statistical power from quintile-binning the dependent variables.
This result reinforces our view that the accessibility variables are
unlikely to be entirely exogenous to the perceived accessibility
indicators and the outcomes analyzed, and that the instrumental variable
framework remains necessary. As is generally the case in instrumental
variable designs \citep{cunningham2021causal, huntington-klein2021}, our
assumption that the instruments affect perceived accessibility and
employment outcome only through calculated accessibility cannot be
directly tested. Even so, adopting an instrumental variable approach was
the most suitable alternative for accounting for potential endogeneity
...
\begin{table}[H]
\centering
\caption{\label{tab:unnamed-chunk-6}\label{tab:iv-diagnostics}Instrument relevance (weak-instrument F-tests) and endogeneity (Wu--Hausman) diagnostics for linear (2SLS) models estimated on the original continuous dependent variables.}
\centering
...
\caption{\label{tab:unnamed-chunk-8}\label{tab:linear_models}Weighted linear regression models connecting calculated to perceived accessibility.}
\centering
\begin{tabular}[t]{lcccccc}
\toprule
...
\caption{\label{tab:unnamed-chunk-10}\label{tab:few_jobs_break_model}Alternative weighted ordinal logistic regression model connecting calculated to perceived accessibility using a fixed-interval categorization for the dependent variable.}
\centering
\begin{tabular}[t]{lccc}
\toprule
