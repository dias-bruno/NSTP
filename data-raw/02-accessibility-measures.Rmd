...
specific_function_final <- specific_function_final %>% 
  mutate(PwMode = case_when(PwMode_label == "Car-motorized" ~ "Car-motor", 
...
generate_impedance <- function(mode_function, travel_cost){
  ...
  # lnorm impedance 
  if(mode_function$Distribution == "Log-normal"){
  ...
  # Unif impedance 
  else if(mode_function$Distribution == "Uniform") {
   f <- dunif(travel_cost,
                     min = 0,
                     max = mode_function$est_2)
   
  # Exponential impedance 
  } else if(mode_function$Distribution == "Exponential") {
  ...
  # Norm impedance 
  } else if (mode_function$Distribution == "Normal"){
        f <- dnorm(travel_cost,
...
```{r}
SA_pp_long_summary <- SA_pp_long %>% 
  mutate(PwMode_label = factor(PwMode_label)) 


SA_pp_long_summary %>%
  dplyr::group_by(PwMode_label) %>%
  dplyr::summarise(mean_val = mean(SA_pp),      
    median_val = median(SA_pp),
    min_val = min(SA_pp),
    max_val = max(SA_pp))
```
...
```{r out.width="100%"}
mode_metrics_active <- mode_metrics_ct_sf %>%
  mutate(PwMode_label = case_when(PwMode_label %in% c("Bike", "Walk") ~ "Active",
    TRUE ~ PwMode_label )) %>%
  group_by(CTUID, PwMode_label) %>%
  summarise(labour_force = sum(labour_force, na.rm = TRUE), 
            across(-labour_force, ~ first(.x)), 
            .groups = "drop") %>% 
  mutate(Pop_density = round(labour_force/LANDAREA))

total_pop_by_mode_CT_3 <- tm_shape(mode_metrics_active) + 
   tm_polygons("labour_force",
              style = "jenks",
              palette = "Reds",
              title = "Labour force",
...
tmap_save(total_pop_by_mode_CT_3, 
          paste0(diretorio_export_figures,"total_pop_by_mode_CT_3.jpg"), 
          width = 14, 
          height = 14, 
          units = "cm", 
          dpi = 300)

knitr::include_graphics(paste0(diretorio_export_figures,"total_pop_by_mode_CT_3.jpg"))
```
...
```{r out.width="100%"}
SA_by_mode_active <- mode_metrics_ct_sf %>%
  mutate(PwMode_label = case_when(PwMode_label %in% c("Bike", "Walk") ~ "Active",
    TRUE ~ PwMode_label )) %>%
  group_by(CTUID, PwMode_label) %>%
  summarise(SA_im = sum(SA_im, na.rm = TRUE), 
            across(-SA_im, ~ first(.x)), 
            .groups = "drop")

SA_by_mode_CT_active <- tm_shape(SA_by_mode_active) + 
   tm_polygons("SA_im",
              style = "quantile",
              palette = "Oranges",
...
tmap_save(SA_by_mode_CT_active, 
          paste0(diretorio_export_figures,"SA_by_mode_CT_active.jpg"), 
          width = 14, 
          height = 14, 
          units = "cm", 
          dpi = 300)

knitr::include_graphics(paste0(diretorio_export_figures,"SA_by_mode_CT_active.jpg"))
```
...
SA_pp_3modes_v2 <- mode_pp_metrics_ct_sf %>% 
  filter(PwMode_label %in% c("Active", "Transit", "Car-motor")) %>%
  mutate(PwMode_label = case_when(PwMode_label == "Car-motor" ~ "Private vehicles",
                                  PwMode_label == "Active" ~ "Active modes",
...
mode_metrics_ct <- mode_metrics_ct %>%
  mutate(PwMode_label_active = PwMode_label,
         PwMode_label_active = case_when(PwMode_label_active %in% c("Bike","Walk") ~ "Active",
                                         PwMode_label == "Car-motor" ~ "Motorized",
                                         TRUE ~ PwMode_label_active)) 

modal_SA_percents <- mode_metrics_ct %>% 
  group_by(PwMode_label_active) %>%
...
Pop_SA_by_mode_CT <- ggplot(modal_SA_long, 
      aes(x = perc, y = measure, fill = PwMode_label_active, label = measure %>% percent(accuracy = 0.1))) +
