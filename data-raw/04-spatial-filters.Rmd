...
SA_pp_mode <- read.csv(paste0(measures_folder, "SA_mode.csv"), 
               colClasses = c("character", "factor", "numeric", "numeric", "numeric")) %>%
  mutate(SA_pp_im = SA_im/labour_force, 
         SA_pp_im = ifelse(is.nan(SA_pp_im), 0, SA_pp_im)) %>%
  dplyr::select(-c(SA_nonc_im, labour_force, SA_im)) %>% 
  pivot_wider(names_from = PwMode_label, values_from = SA_pp_im, names_sep = "_", values_fill = 0) %>% 
  mutate(Active = Walk + Bike) %>% 
  rename(CTUID = from_id)
...
SA_pp_general <- read.csv(paste0(measures_folder, "SA_general.csv"), 
  ...
  rename(CTUID = from_id) %>% 
  mutate(General = SA_i/labour_force) %>% 
  dplyr::select(c(CTUID, General))  

SA_pp <- SA_pp_mode %>% 
  left_join(SA_pp_general, by = "CTUID") %>% 
  pivot_longer(cols = c(Bike, Walk, `Car-motor`, Transit, Active, General), 
               names_to = "PwMode_label",
               values_to = "SA_pp_im")
```
